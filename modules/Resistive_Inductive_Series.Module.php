...
class Resistive_Inductive_Series extends FormulaBase{
    function __construct(){
        $this->entries = array(
            ...
            FormulaBase::create_func_entry(
                "Inductor VAR's using Indcutor Volts and Inductive Reactance",
                function($num, $num2){
                    $result = pow($num, 2) / $num2;
                    return array($this->prec($result, 4), $this->pluralize($result, 'Inductor VAR'));
                },
            ...
            FormulaBase::create_func_entry(
                "Inductor Volts using Total Volts and Resistor Volts",
                function($num, $num2){
                    $result = pow($num, 2) - pow($num2, 2);
                    if ( $result <= 0 ){
                        return array($this->error_msg, '');
            ...
            FormulaBase::create_func_entry(
                "Resistor Volts using Total Volts and Inductor Volts",
                function($num, $num2){
                    $result = pow($num, 2) - pow($num2, 2);
                    if ( $result <= 0 ){
                        return array($this->error_msg, '');
            ...
            FormulaBase::create_func_entry(
                "Total Volts using Resistor Volts and Inductor Volts",
                function($num, $num2){
                    $result = sqrt( pow($num, 2) + pow($num2, 2) );
